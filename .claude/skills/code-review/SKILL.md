...
argument-hint: "Optional: the files or plan section to scope the review to"
user-invocable: true
context: fork
background: false
...
**Default to subtask scope, not full-branch scope.** When reviewing as part of the gated Task Execution Workflow (see [AGENTS.md](../../../AGENTS.md)), the caller will give you:
...
4. **Architecture** — evaluate cohesion, coupling, file size, and placement:
   ...
   - **`lib/` internal organization** — `lib/` is organized **by concept, not by execution context** (full rules in the **"`lib/` internal organization"** section of [AGENTS.md](../../../AGENTS.md)). When the change adds or moves a file under `lib/`, verify:
