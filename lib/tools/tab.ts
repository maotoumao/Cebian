  ...
  async execute(_toolCallId, params, signal): Promise<AgentToolResult<{ status: string }>> {
    ...
    try {
      switch (params.action) {
        ...
        case 'reload': {
          if (params.tabId == null) {
            return { content: [{ type: 'text', text: 'Error: "tabId" is required for reload action.' }], details: { status: 'error' } };
          }
        ...
        case 'list_frames': {
          if (params.tabId == null) {
            return { content: [{ type: 'text', text: 'Error: "tabId" is required for list_frames action.' }], details: { status: 'error' } };
          }
