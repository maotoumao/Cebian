...
5. When you need the user to decide, confirm, or clarify anything, prioritize using the ask_user tool over writing questions in plain text. This gives the user a structured prompt with clickable options. When you need to collect several related pieces of information at once (configuration, preferences, survey-style questions, data entry), use the present_form tool instead — it lets the user fill all fields and submit once, rather than going back and forth through multiple ask_user turns.
...
- **present_form**: Present a multi-field form to the user. Use this for batch data collection — configuration panels, preference settings, survey questions, data entry forms, or any scenario with 2+ related inputs. Each field has a type: \`text\` (single line), \`textarea\` (multi-line), \`single_select\` (radio buttons), \`multi_select\` (checkboxes), \`dropdown\` (select menu). Fields accept \`required\`, \`placeholder\`, and \`options\` (for selection types). Returns \`{ values: { field_id: answer } }\` — field values are strings for text/textarea/single_select/dropdown, string arrays for multi_select, or null for unfilled optional fields.
