...
- 升级核心 AI 引擎 pi-ai / pi-agent-core 至 0.81.1，跟进上游模型目录更新（新增 Kimi K3 系列可选模型、调整 xAI/Grok 目录），并带来一批服务商健壮性修复（Kimi K3 思考格式与推理档校正、Cloudflare、OpenAI Codex 等）
- 思考档选择器现在按当前模型动态显示它真正支持的档位：不支持的档位不再出现（如无法关闭思考的模型不再显示「关闭」），支持的模型会多出「极高 / 最大」两档；已选档位超过所换模型上限时会自动夹到该模型的可用档并按此发送，让显示与实际生效保持一致

- Upgraded the core AI engine (pi-ai / pi-agent-core) to 0.81.1, picking up upstream model-catalog refreshes (new selectable Kimi K3 models and an adjusted xAI/Grok catalog) and a batch of provider robustness fixes (Kimi K3 thinking-format and reasoning-level corrections, Cloudflare, OpenAI Codex, etc.)
